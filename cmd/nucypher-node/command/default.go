...
func (dc *DefaultCommand) updateCache(period string, sendedTx *types.Transaction) error {
	dc.cache.Period = period
	dc.cache.TxHash = sendedTx.Hash().String()
	dc.cache.Nonce = sendedTx.Nonce()
	dc.cache.GasPrice = sendedTx.GasPrice().String()
	data, err := rlp.EncodeToBytes(sendedTx)
	if err != nil {
		return err
	}
	dc.cache.TxHex = hexutil.Encode(data)
	dc.cache.StartTime = time.Now().UnixNano()
	return nil
}
